...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item[1.]
        \textparagraph 等号两边的结果应是一个矢量。以$\hat{i}$方向分量为例：
        ...
        \textparagraph 根据定义，由于$\bm{r}$的三个分量相互独立，应有：
        \begin{align*}
            \nabla\times\bm{r}
            &= (\partial_j r_k - \partial_k r_j)\hat{i} + (\partial_k r_i - \partial_i r_k)\hat{j} + (\partial_i r_j - \partial_j r_i)\hat{k} \\
            &= (0-0)\hat{i} + (0-0)\hat{j} + (0-0)\hat{k} \\
        ...
        \textparagraph 结果应是一个矢量，以$\hat{i}$方向分量为例：
        \begin{align*}
            (\bm{a}\cdot\nabla)\bm{r}
        ...
        \textparagraph 注意到$\sin(\bm{k}\cdot\bm{r})]$是一个标量函数。根据公式$\nabla\times(f\bm{A}) = f(\nabla\times\bm{A}) + (\nabla f)\times\bm{A}$得：
        \[
        ...
        \item[7.(2)]
        \textparagraph 极化体电荷只分布在介质内部，设其体密度为$\rho_p$。
        ...
        \textparagraph 极化面电荷只分布在界面$r = r_1$及$r = r_2$，设其面密度为$\sigma_p$。
        ...
        \item[8.(2)]
        \textparagraph 磁化体电流只分布在介质内部，设其面密度为$J_m$。
        ...
        \textparagraph 磁化面电流只分布在界面$r = r_1$及$r = r_2$，设其面密度为$\bm{\alpha}_m$。
        ...
        \item[11.(2)]
        \textparagraph 在介质内部，由$\bm{D} = \varepsilon\bm{E}$得：
        \[
        ...
        \textparagraph 若介质漏电，则会产生沿$\hat{z}$方向的电流$\bm{J}$。导体极板内有电流，$\omega_{f1} + \omega_{f2} \neq 0$
    ...
    \begin{itemize}
        ...
        \]
        而该球中的空洞可以看作是原有的半径$r=R$，电荷密度为$\rho$的带电球与一个半径$r=\dfrac{R}{2}$，电荷密度为$-\rho$的带电球叠加。

        \begin{center}
            \begin{tikzpicture}[>=Stealth]
                \draw (0,0) circle [radius=3cm];
                \draw (1.5,0) circle [radius=1.5cm];
                \draw [->] (0,0) -- (-2.5980762,-1.5) node [midway,above] {$R$};
                \draw [->] (1.5,0) -- (2.7990381,0.75) node [midway,below] {$\frac{R}{2}$};
                \draw [->] (0,0) -- (1.2,-0.8) node [midway,below] {$\bm{r}$};
                \draw [->] (1.5,0) -- (1.2,-0.8) node [midway,right] {$\bm{r'}$};
                \draw [->] (0,0) -- (1.5,0) node [midway,above] {$\bm{r}_0$};
                \fill (0,0) circle [radius=2pt] node [anchor=south east] {$O$};
                \fill (1.5,0) circle [radius=2pt] node [above] {$O'$};
                \fill (1.2,-0.8) circle [radius=2pt] node [below] {$P$};
            \end{tikzpicture}
        \end{center}

        如图，空洞内部任意一点$P$的电场强度为：
        \[
            \bm{E} = \frac{\rho\bm{r}}{3\varepsilon_0} - \frac{\rho\bm{r}'}{3\varepsilon_0} = \frac{\rho}{3\varepsilon_0}(\bm{r}-\bm{r}')
        \]
        其中$\bm{r}$为$O$指向$P$的矢量，$\bm{r}'$为$O'$指向$P$的矢量。

        则$\bm{r}-\bm{r}'=\overrightarrow{OP}-\overrightarrow{O'P}=\overrightarrow{OO'}$，记作$\bm{r}_0$，为一大小为$\dfrac{R}{2}$的常矢量。

        于是空洞内部的电场为一均匀电场，大小为：
        \[
            \bm{E} = \frac{\rho R}{6\varepsilon_0}
        \]
        方向沿$\overrightarrow{OO'}$。
