...
\begin{document}
    ...
    2,8,20,28,50,82,126
    \]
    实验表明，当中子数$N$或质子数$Z$取这些“幻数”（质子取前6项）时，原子核的中子分离能或质子分离能会取极大值：
    \[
    ...
    \]
    这表明$N$或$Z$取“幻数”的“幻核”最稳定。而两者都取幻数的“双幻核”，如$\mathrm{^{4}He}$、$\mathrm{^{16}O}$、$\mathrm{^{208}Pb}$等，尤其稳定。量子力学告诉我们元素的化学稳定性与核外电子的壳层排布有关，电子充满各级壳层时元素化学性质最稳定（即稀有气体）。与元素周期律中电子壳层排布与化学性质的关系类比，不难联想到原子核的稳定性规律也可能与原子核的壳层结构有关。所以，核子“幻数”的存在最直接地表明，原子核中核子能级存在壳层结构。
    ...
    \begin{figure}[!ht]
        \centering
        \includegraphics[width = 0.9\textwidth]{nucpot.png}
        \caption{核子感受到的势}
    \end{figure}
    ...
    \[
    u_l(\xi) = P \cdot \mathrm{F}(\alpha,\gamma,\xi) \xi^{1-\gamma} + Q \cdot \mathrm{F}(\alpha-\gamma+1,2-\gamma,\xi)
    \]
    ...
    \]
    可以看出，$\mathrm{F}(\alpha,\gamma,\xi)$一般是无穷级数，在$\xi \to \infty$时发散行为与$\mathrm{e}^\xi$相同，欲满足$\xi \to \infty$时$u_l(\xi)$有限，必须使无穷级数截断为一个多项式，这就要求$\alpha \leqslant 0$，即：
    \[
    \alpha = \frac{1}{2}\left(l + \frac{3}{2} - E\right) = -n_r,\qquad n_r \in \mathbb{N}
    \]
    其中$n_r$为径向量子数。
    ...
    令$N = 2n_r + l$，则：
    \[
    E = E_N = \left(N + \frac{3}{2}\right)\hbar\omega,\qquad N \in \mathbb{N}
    \]
    ...
    \[
    E(r) = \lambda(r)\bm{s}\cdot\bm{l},\qquad \lambda(r) = \frac{1}{2\mu^2 c^2 r}\cdot\frac{\mathrm{d}V}{\mathrm{d}r}
    \]
    ...
    \Delta E
    &= E_{j=l+s} - E_{j=l-s} \\
    &= \langle\lambda(r)\rangle \cdot \left\{\langle\bm{s}\cdot\bm{l}\rangle_{j=l+s} - \langle\bm{s}\cdot\bm{l}\rangle_{j=l-s}\right\} \\
    ...
    \begin{figure}[!ht]
        \centering
        \includegraphics[width=0.9\textwidth]{shellmod.png}
        \caption{原子核壳层模型示意图}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item[1.] Island of Stability -- Wikipedia, \texttt{https://en.wikipedia.org/wiki/Island\_of\_stability}

        \item[2.] Nuclear Shell Model -- Wikipedia, \texttt{https://en.wikipedia.org/wiki/Nuclear\_shell\_model}

        \item[3.] Magic Number (Physics) -- Wikipedia, \texttt{https://en.wikipedia.org/wiki/Magic\_number\_(physics)}

        \item[4.] Nave, C. R. \textit{Shell Model of Nucleus}. HyperPhysics.

        \item[5.] Mayer, Maria G. (1948). \textit{On Closed Shells in Nuclei}. Physical Review.

        \item[6.] Mayer, Maria G. (1949). \textit{On Closed Shells in Nuclei. II}. Physical Review.

        \item[7.] Zagrebaev, V.; Karpov, A.; Greiner, W. (2013). \textit{Future of superheavy element research: Which nuclei could be synthesized within the next few years?} Journal of Physics.

        \item[8.] \'{C}wiok, S.; Nazarewicz, W.; Heenen, P. H. (1999). \textit{Structure of Odd-N Superheavy Elements}. Physical Review Letters.
