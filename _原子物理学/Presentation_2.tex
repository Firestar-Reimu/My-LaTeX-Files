...
在大爆炸后$\sim10^{-6}\,\mathrm{s}$，宇宙温度$T\sim1\,\mathrm{GeV}$时，夸克脱耦（夸克——强子转变），此时夸克无法自由存在，并通过胶子相互结合成强子（重子和介子），中子就是在这个过程中产生的。这时中子与质子的数目比仍接近$1:1$。
...
\]
即此时中子与质子的数量比约为$1:7$。由于几乎所有的中子经过原初核合成都转移到$^{4}\mathrm{He}$中，$^{4}\mathrm{He}$的相对丰度为：
\[
...
Introduction to Cosmology, Max Pettini, Cambridge University.
