...
\begin{slide}{\url{https://www.azom.com/article.aspx?ArticleID=11451}}
    ...
    另一种半导体材料砷化镓在1962年被用于制造红外LED，其直接带隙为$1.4\,\mathrm{eV}$，落在红外区。还有一种半导体材料磷化镓，适当掺杂做成二极管可以使其发出红光和绿光。而对蓝光而言，就没有那么容易了。
\end{slide}
...
\begin{slide}{\url{https://www.azom.com/article.aspx?ArticleID=11451}}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.6\columnwidth]{pictures/GaN.pdf}
    \end{figure}
    \bigskip
    ...
    \end{multicols}
    {\tiny 图片来源：\url{https://en.wikipedia.org/wiki/File:GaN_Wurtzite_polyhedra.png}
}
\end{slide}
...
\begin{slide}
    ...
    \kaishu
    \raggedright 我可以有把握地说，没人懂得量子力学。

    \raggedleft ——理查德·费曼
    \bigskip

    \raggedright 量子力学绝对是上个世纪最伟大的发现，没有之一。没人能弄懂量子力学，但所有这些半导体元件都是量子力学创造出来的。看看，我们的手机，我们的电脑，里面有多少三极管。

    \raggedleft ——赵建业（学生回忆版）
    \end{multicols}
    \scriptsize{图片来源: \url{https://pkuhelper.pku.edu.cn/hole}}
\end{slide}
...
\begin{slide}{RGB系统}
    ...
    没有荧光粉的多色LED无法提供良好的显色性，因为每个LED都是窄带光源。这虽然是一种较差的普通照明方案，但对于基于LED直接像素的显示器来说，这是最佳的解决方案。
\end{slide}
...
\begin{slide}
    ...
    \textit{发光二极管所需推动电压及功率低，方便由运作电压低的微处理器控制及在以电池作电源的设备上使用，所以常被用在各种电子产品、设备的状态指示灯。在消费性电子产品如家庭电器、儿童玩具、电路开关等上作为工作状态显示灯。}
    \begin{figure}[h]
        \centering
...
\begin{itemize}
    ...
    \textit{当LED的光度发展达到要求后，便有汽车用LED作转向与刹车灯，主要好处是LED极高的开关速度，其亮起时间比白炽灯快$0.5\,\mathrm{s}$之多，且LED灯在白天时与白炽灯相比亮度较高、使用寿命长、点亮速度快，更可增加$6\,\mathrm{m}$的刹车辨识距离，降低事故发生的概率，这对行车安全非常重要。而在车辆行驶环境中震荡、温差变化下，LED仍然有稳定的光度及可靠性，可见LED灯是车灯的理想选择。}
\end{itemize}
\end{slide}
...
\begin{itemize}
    ...
    \item \textit{大型的LED显示器已普及于户内、户外。户外LED显示器对解析度要求较低，但需要较高的亮度，多采用分立单色的LED组成。}
\end{itemize}
\end{slide}
