...
\paragraph*{注意} 此处能量$\epsilon$和角频率$\omega$都用波数$\tilde{\nu}=\dfrac{1}{\lambda}$表示，如果定义$k=\dfrac{k_\mathrm{B}}{hc}=0.69503476\,\mathrm{cm}^{-1}$，对于能量：
\[
...
\begin{enumerate}
    ...
    \[
        \frac{N_J}{N_0} = (2J+1)\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}}
    \]
    由$\dfrac{\mathrm{d}}{\mathrm{d}x}\mathrm{e}^{a(x)} = a'(x)\mathrm{e}^{a(x)}$，对$J$求偏导得：
    \begin{align*}
        \frac{\partial}{\partial J}\left(\frac{N_J}{N_0}\right)
        &= 2\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}} + (2J+1) \cdot \left[-\frac{B(2J+1)}{kT}\right]\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}} \\
        &= \left[-\frac{B(2J+1)^2}{kT} + 2\right]\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}}
    \end{align*}
    \begin{align*}
        \frac{\partial^2}{\partial J^2}\left(\frac{N_J}{N_0}\right)
        &= -\frac{4B(2J+1)}{kT}\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}} + \left[-\frac{B(2J+1)^2}{kT} + 2\right] \cdot \left[-\frac{B(2J+1)}{kT}\right]\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}} \\
        &= \left[\frac{B^2 (2J+1)^3}{k^2 T^2} - \frac{6B(2J+1)}{kT}\right]\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}}
    \end{align*}
    当$\dfrac{\partial}{\partial J}\left(\dfrac{N_J}{N_0}\right) = 0$时，有$\dfrac{2kT}{B} = (2J+1)^2$，即：
    \[
        J = \sqrt{\frac{kT}{2B}} -\frac{1}{2}
    \]
    此时$\dfrac{\partial^2}{\partial J^2}\left(\dfrac{N_J}{N_0}\right) = -\dfrac{4B(2J+1)}{kT}\mathrm{e}^{-\tfrac{BJ(J+1)}{kT}} < 0$，说明$\dfrac{N_J}{N_0}$达到最大值。
    ...
    \]
    且$\dfrac{N_2}{N_1} = \dfrac{5}{3}\mathrm{e}^{-\tfrac{4B}{kT}}=0.9564$，则处于转动能级$J = 1$的分子最多。
    ...
    \]
    且$\dfrac{N_7}{N_6} = \dfrac{15}{13}\mathrm{e}^{-\tfrac{14B}{kT}}=1.0136$，则处于转动能级$J = 7$的分子最多。
    ...
    \]
    且$\dfrac{N_23}{N_22} = \dfrac{47}{45}\mathrm{e}^{-\tfrac{46B}{kT}}=1.0009$，则处于转动能级$J = 23$的分子最多。
    ...
    \item[4.(b)]
    对$\Delta E_{5 \to 4}$的估计更准确，因为随着$J_i$增大，离心力的影响更加显著，能量差逐渐减小。

    \item[5.]
    $\mathrm{CH}^+$的$J=1$到$J=0$的跃迁发射线频率为$\nu_1=835.07\,\mathrm{GHz}$

    又因为$\Delta E_{i \to i-1} = 2B_0 J_i$：
    \[
    ...
    振动跃迁频率$f^\mathrm{vib} = \hbar\omega(v_1 - v_0) = \hbar\omega$，假设两种分子具有相同的等效劲度系数$k$，则$\omega = \sqrt{\dfrac{k}{\mu}} \approx \mu^{-\tfrac{1}{2}}$，故$f^\mathrm{vib} \approx \mu^{-\tfrac{1}{2}}$
